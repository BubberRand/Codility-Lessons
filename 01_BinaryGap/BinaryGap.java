...
public class BinaryGap {
    public static void main(String[] args) 
    {
        ...
        System.out.println(binaryGapFun(numero));
    ...
    public static int binaryGapFun(int n) 
    {
        ...
        //"0+$" is replacing all trailing zeros, regex is whack
        String[] gaps = binaryNum.replaceAll("0+$", "").split("1");

        int longestGap = 0;
        for(String i: gaps)
        {
            if(i.length() > 0 && i.length() > longestGap)
            {
                longestGap = i.length();
            }
        }
